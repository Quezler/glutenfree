...
script.on_event(defines.events.on_script_trigger_effect, function(event)
  ...
  if struct.silo.valid then
    ...
    if inserted > 0 then
      ...
      assert(struct.silo.rocket_parts == new_rocket_parts, string.format("changed %d to %d but it stayed at %d.", old_rocket_parts, new_rocket_parts, struct.silo.rocket_parts))
    end
  else
