...
function train.entangle_with_station(train, station)
  if not storage.entangled[train.id] then storage.entangled[train.id] = {} end

  storage.entangled[train.id][station.unit_number] = station
end
...
function train.entangled_with_stations(train)
  ...
  for unit_number, station in pairs(storage.entangled[train.id] or {}) do
  ...
  storage.entangled[train.id] = nil
