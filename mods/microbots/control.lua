  ...
  while true do
    ...
    for slot = 1, #inventory do
      ...
      local robot = surface.create_entity{
        name = 'invisible-construction-robot',
        position = position,
        force = force,
      }

      robot.get_inventory(defines.inventory.robot_cargo).insert(itemstack)

      -- game.print(robot.logistic_network.cells[1].owner.force.name)
      -- game.print(game.get_player(1).character.logistic_network.cells[1].owner.force.name)
      robot.logistic_network = game.get_player(1).character.logistic_network
